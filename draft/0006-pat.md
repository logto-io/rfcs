...
1. `name`: REQUIRED. A user-defined name for the PAT to identify its purpose, should be unique in the user's account.
...
{
  "name": "My PAT",
  "token": "pat_W51arOqe7nynW75nWhvYogyc",
}
```
...
5. `subject_token_type`: REQUIRED. The type of the security token provided in the `subject_token` parameter. The value of this parameter must be `urn:logto:token-type:personal-access-token`.

For security reasons, the client must authenticate with the token endpoint using the client ID and client secret in the `Authorization` header. Which means the client must be a traditional web application.

And organization resources are not available for PATs.
...
&subject_token_type=urn%3Alogto%3Atoken-type%3Apersonal-access-token
```
...
1. PATs may be misused or leaked if not monitored or revoked in a timely manner.
2. PATs may be used to bypass multi-factor authentication if not properly enforced.

## Alternatives

1. Implement a separate authentication flow for non-interactive clients, which may require additional development effort.
2. Use long-lived access tokens, which may be less secure and harder to manage.

## Future Possibilities

1. Support for PAT rotation and automatic expiration policies.
2. Fine-grained control over PAT permissions and scopes.
3. Webhooks for PAT-related events to help users monitor their token usage.
