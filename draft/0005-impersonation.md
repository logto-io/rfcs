...
4. `subject_token`: REQUIRED. A security token that represents the identity of the party on behalf of whom the request is being made. See the next section for more details.
5. `subject_token_type`: REQUIRED. The type of the security token provided in the `subject_token` parameter. The value of this parameter must be `urn:ietf:params:oauth:token-type:access_token`.
6. `actor_token`: OPTIONAL. A security token that represents the identity of the acting party, it should contain valid `accountId` and have `openid` scope. If present, the access token issued by Logto will contain an additional `act` claim that represents the identity of the acting party.
7. `actor_token_type`: OPTIONAL. The type of the security token provided in the `actor_token` parameter. The value of this parameter must be `urn:ietf:params:oauth:token-type:access_token`.
...
#### Example subject token issuance
...
```
POST /api/security/subject-tokens HTTP/1.1
Host: example.logto.app
...
## Additional claims in the access token
...
The exchanged access token will contain a `client_id` claim that represents the client application that requested the token. When a subject token is issued, the `client_id` claim in the subject token is set to the `client_id` of the machine-to-machine application that requested the subject token.
...
The support engineer exchanges the subject token for an access token that represents the identity of the user:
